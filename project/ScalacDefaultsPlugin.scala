...
object ScalacDefaultsPlugin extends AutoPlugin {
  ...
  val scalacOptions2_12_2 = List(
    // See: https://github.com/scala/scala/blob/2.12.x/src/compiler/scala/tools/nsc/settings/Warnings.scala#L18
    "-Ywarn-extra-implicit" // Warn when more than one implicit parameter section is defined.
  )

  def scalacOptionsFor212PointReleases(scalaVersion: String): List[String] = {
    val pointRelease = scalaVersion.substring("2.12.".length).toInt
    if (pointRelease >= 2)
      scalacOptions2_12_2
    else
      Nil
  ...
  def scalacOptionsForVersion(scalaVersion: String): List[String] =
    CrossVersion.partialVersion(scalaVersion) match {
      case Some((2, 10)) => scalacOptions210
      case Some((2, 11)) => scalacOptions211
      case Some((2, 12)) => scalacOptions2_12_0 ++ scalacOptionsFor212PointReleases(scalaVersion)
      case _             => Nil
    }
